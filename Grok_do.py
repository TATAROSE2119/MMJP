...
# 5. 模拟PMD在线模式识别（生成图4，保持不变）
def plot_mode_identification(test_case, case_num, n_samples):
    if case_num == 1:
        mode_ids = np.array([1] * 200 + [2] * 200)  # Case 1: Mode 1 -> Mode 2
    else:  # case_num == 2
        mode_ids = np.array([1] * 200 + [2] * 400 + [3] * 300)  # Case 2: Mode 1 -> Mode 2 -> Mode 3
...
if __name__ == "__main__":
    ...
    # 生成并可视化测试数据（图4，Case 1）
    plot_mode_identification(test_case1, 1, 400)

    # 生成并可视化测试数据（图4，Case 2）
    plot_mode_identification(test_case2, 2, 900)